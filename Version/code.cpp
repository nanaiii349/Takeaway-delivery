...
orderList input()
{
    ...
    lastPtr = headPtr;
    int a, b, c, d, e, f;
    FILE *fp = fopen("C:\\Users\\admin\\Desktop\\adjust_1.txt", "r");
    if (fp == NULL)
        printf("error open!\n");
    else
    {
...
void output() //输出
{
    ...
    FILE *fp;
	fp=fopen("C:\\Users\\admin\\Desktop\\10组输出\\\1.txt","a");
    if (RUN == 3)
    {
...
mapNode search(int posx, int posy, int num2, int curPosX, int curPosY, int dir, int dis, int no) //对当前扫描点进行检查
{
    ...
    {
        printf("WRONG POSITION IN BFS FOR NO:%d RIDER!\n", no);
        RUN = 3;
    }
    return keyNode;
}
